...
public class RabbitMqConsumer : BackgroundService
{
    ...
    public RabbitMqConsumer(IOptions<RabbitMqOptions> rabbitMqOptions, IServiceProvider serviceProvider)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            try
            {
                ...
                var recipientNotification = messageParts[2];

                using var scope = _serviceProvider.CreateScope();
                var emailService = scope.ServiceProvider.GetRequiredService<IEmailService>();
                await emailService.SendEmail(recipientEmail, subject, recipientNotification);
